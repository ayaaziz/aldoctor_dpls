...
export class FinancialPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,
    ...
    public loginservice: LoginServiceProvider) {
      ...
      //ayaaaa //renew subscripton date
      this.storage.get("user_login_token").then((val) => {
        this.loginservice.getsubscriptionDate(val.access_token).subscribe(
          resp => {
            console.log(JSON.stringify(resp));

            let data = JSON.parse(JSON.stringify(resp));
            this.subscribe_date_str = data.RenewSubscriptionDate;

            console.log("this.subscribe_date_str: "+this.subscribe_date_str);
          },
          error => {
