...
/**
 * Generated class for the LabRegistrationPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-lab-registration',
  templateUrl: 'lab-registration.html',
  providers: [FileChooser, Base64, FilePath, IOSFilePicker, Camera, File]
})
export class LabRegistrationPage {
    ...
    if (this.editmode) {
      ...
      this.storage.get("user_login_info").then((val) => {
        if (val != null) {
          ...
          this.labName = val.name
          this.labOwner = val.extraInfo.owner_name
  ...
  ionViewDidLoad() {
    ...
    this.loginservice.getLabTypes(this.helper.currentLang, (data) => { this.ServicesData = data }, (data) => { this.helper.presentToast(this.translate.instant('serverError')) })
  }
  registerUser() {
    ...
    if (this.termsStatus == false) {
      ...
      if (this.registerFormOne.valid) {
        ...
        if (navigator.onLine) {
          if (this.editmode == 1) {
            serviceData.name = this.labName
            serviceData.email = this.email
            serviceData.address = this.address
            serviceData.owner = this.labOwner
  ...
  authSuccessCallback(data) {
    ...
    if (navigator.onLine) {
      this.loginservice.entityRegister(this.labName, this.email, "2" + this.phone, this.entityphone, this.logoImg, this.profile_pic_ext, this.passwordTxt, String(this.certificateData), this.address, this.extra_info, String(this.certifications_ext), this.labOwner, 1, 3, this.taxLicense, this.governorate, this.city, this.deliveryCount, this.services, (data) => this.successCallback(data), (data) => this.failureCallback(data))
    }
    else {
      this.helper.presentToast(this.translate.instant("serverError"))
    }
