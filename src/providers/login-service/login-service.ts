...
getCities(id,lang, successCallback , failCallback){
  this.http.get(this.helper.serviceUrl+'api/get/lkps/cities?governerate_id='+id+'&patient=1').subscribe(data => successCallback(data) , err => failCallback(err))
...
userLogin(email,password,categoriesSuccessCallback,categoriesFailureCallback) {
  ...
  //key=AIzaSyCOlXpEUuoXStcUGDGXfFx_axfNfi4CkvY&language=ar
    ...
    userEditProfile(first_name,second_name,last_name,nickname,union_id,birth_date,college_id,graduation_year,bio,bio_ar,certifications,speciality_services,email,gender,speciality_id,address,terms,educational_degree,access_token,certifications_ext,city_id,categoriesSuccessCallback,categoriesFailureCallback) {
      let loader = this.loadingCtrl.create({
        content: "",
      });
       loader.present();
  ...
  registerFirebase(firebase_registeration_id, access_token) {
    ...
    getToolsForNursing(lang,SuccessCallback,FailureCallback){ 
      let headers = new HttpHeaders();
      let loader = this.loadingCtrl.create({
        content: "",
      });
      ...
      serviceUrl = this.helper.serviceUrl +'api/get/lkps/nursing-speciality-tool?lang='+lang+'&speciality_id='+0;
...
//api/notifications/read/{id} (post)
  ...
  //ayaaaaaaa
  editserviceProfileNursing(data,access_token){
    let headers = new HttpHeaders();
    if(!data.entity_services){
      data.entity_services = ""
    }
...
uploadReport(orderId,access_token,reportFiles,files_ext){
  ...
  getsubscriptionDate(access_token) {
    let headers = new HttpHeaders();
    headers = headers.set('Content-Type', 'application/x-www-form-urlencoded').set('Authorization', 'Bearer '+access_token);
    let serviceUrl = this.helper.serviceUrl+ 'api/subscriptionDate';
    return this.http.get(serviceUrl,{headers: headers });
  }
